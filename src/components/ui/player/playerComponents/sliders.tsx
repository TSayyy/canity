...
export function Volume() {
  return (
    <VolumeSlider.Root className="volume-slider group relative mx-[7.5px] inline-flex h-10 w-full max-w-16 sm:max-w-[80px] cursor-pointer touch-none select-none items-center outline-none aria-hidden:hidden">
      ...
      <VolumeSlider.Thumb className="absolute left-[var(--slider-fill)] top-1/2 z-20 h-[12px] w-[12px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#cacaca] bg-white opacity-0 ring-white/40 transition-opacity group-data-[active]:opacity-100 group-data-[dragging]:ring-4 will-change-[left]" />
    </VolumeSlider.Root>
  );
}
...
export function Time({ thumbnails }: TimeSliderProps) {
  return (
    <TimeSlider.Root className="time-slider group relative mx-[7.5px] inline-flex h-7 sm:h-10 w-full cursor-pointer touch-none select-none items-center outline-none">
        ...
        {(cues, forwardRef) =>
          cues.map((cue) => (
            <div
              ...
              ref={forwardRef}
            >
              <TimeSlider.Track className="relative ring-media-focus z-0 transition-all h-[3px] group-hover:h-[6px] w-full rounded-sm bg-white/30 group-data-[focus]:ring-[3px]">
                <TimeSlider.TrackFill className="bg-royal-blue absolute h-full w-[var(--chapter-fill)] rounded-sm will-change-[width]" />
                <TimeSlider.Progress className="absolute z-10 h-full w-[var(--chapter-progress)] rounded-sm bg-royal-blue/20 will-change-[width]" />
              </TimeSlider.Track>
            </div>
          ))
        }
      ...
      <TimeSlider.Thumb className="absolute left-[var(--slider-fill)] top-1/2 z-20 h-[13px] w-[13px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#cacaca] bg-white opacity-0 ring-white/40 transition-opacity group-data-[active]:opacity-100 group-data-[dragging]:ring-4 will-change-[left]" />
